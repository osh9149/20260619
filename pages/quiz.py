...
DATA_URL = "https://raw.githubusercontent.com/osh9149/20260619/main/ta_20260619190504.csv"
...
history_events = [
    ...
    {"date": "2002-06-22", "event": "월드컵 4강 신화", "hint": "대한민국 축구 역사상 매우 뜨거웠던 순간이에요."},
...
st.markdown('<div class="big-title">🕰️ 역사 속 서울 날씨 맞히기 🌡️</div>', unsafe_allow_html=True)
...
actual_avg = round(row["평균기온"], 1)
actual_min = round(row["최저기온"], 1)
actual_max = round(row["최고기온"], 1)
...
with right:
    ...
    st.progress(min(st.session_state.score / 300, 1.0))
    ...
    if st.button("✅ 이 온도로 도전하기"):
        ...
        if error <= 1:
            ...
            if st.session_state.attempt < 5:
                if guess < actual_avg:
                    st.session_state.hint_message = "📈 힌트: 실제 기온은 네가 고른 온도보다 더 높아요!"
                else:
                    st.session_state.hint_message = "📉 힌트: 실제 기온은 네가 고른 온도보다 더 낮아요!"
                ...
                st.error(f"😢 실패! 정답은 {actual_avg}℃였습니다. 점수 30점 감소!")
...
if st.session_state.answered:
    ...
    month_df = year_df.groupby("월", as_index=False)["평균기온"].mean()
...
with st.expander("🌍 데이터 탐구 모드"):
    ...
    st.markdown("""
    ### 탐구 질문
    - 역사적 사건이 있었던 날의 날씨는 기억에 어떤 영향을 줄까요?
    - 서울의 평균기온은 장기적으로 상승하고 있을까요?
    - 기후 데이터와 역사 수업을 연결하면 어떤 점이 좋을까요?
    """)

st.caption("서울 기온 데이터 기반 역사 날씨 추리 게임")
